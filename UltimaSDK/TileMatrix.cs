...
namespace MapCreator.Engine.UltimaSDK
{
	public sealed class TileMatrix
	{
		...
		private unsafe HuedTile[][][] ReadStaticBlock(int x, int y)
		{
			try
			{
				...
				var lookup = m_StaticIndex[(x * BlockHeight) + y].lookup;
				var length = m_StaticIndex[(x * BlockHeight) + y].length;
		...
		private unsafe Tile[] ReadLandBlock(int x, int y)
		{
			...
			if (m_Map != null)
			{
				_ = m_Map.Seek((((x * BlockHeight) + y) * 196) + 4, SeekOrigin.Begin);
	...
	public struct MTile : IComparable
	{
		...
		public int Flag { get => m_Flag; set => m_Flag = (sbyte)value; }
