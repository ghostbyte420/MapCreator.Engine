...
namespace MapCreator.Engine.UltimaSDK
{
	...
	public sealed class TileData
	{
		...
		/// </summary>
		public static LandData[] LandTable { get; set; }

		/// <summary>
		/// Gets the list of <see cref="ItemData">item tile data</see>.
		/// </summary>
		public static ItemData[] ItemTable { get; set; }
		...
		{
			_ = bin.Read(m_StringBuffer, 0, 20);
			...
			using (var fs = new FileStream(FileName, FileMode.Create, FileAccess.Write, FileShare.Write))
			{
				using (var bin = new BinaryWriter(fs))
				{
					...
					for (var i = 0; i < 0x4000; ++i)
					{
						...
						{
							var bb = Encoding.Default.GetBytes(LandTable[i].Name);
							if (bb.Length > 20)
							{
					...
					for (var i = 0; i < ItemTable.Length; ++i)
					{
						...
						{
							var bb = Encoding.Default.GetBytes(ItemTable[i].Name);
							if (bb.Length > 20)
							{
								Array.Resize(ref bb, 20);
							}
		...
		public static int ConvertStringToInt(string text)
		{
			...
			if (text.Contains("0x"))
			{
				...
				_ = Int32.TryParse(convert, System.Globalization.NumberStyles.HexNumber, null, out result);
			}
			else
			{
				_ = Int32.TryParse(text, System.Globalization.NumberStyles.Integer, null, out result);
			}
